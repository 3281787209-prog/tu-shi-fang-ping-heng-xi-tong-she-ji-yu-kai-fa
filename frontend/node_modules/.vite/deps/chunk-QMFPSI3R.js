...
function vtkShader(publicAPI, model) {
  ...
  publicAPI.compile = () => {
    ...
    if (!isCompiled) {
      ...
      vtkErrorMacro2(`Error compiling shader '${model.source}': ${lastError}`);
      model.context.deleteShader(model.handle);
      model.handle = 0;
      return false;
    }
...
var newInstance5 = macro.newInstance(extend4, "vtkShader");
var vtkShader$1 = {
...
function vtkShaderProgram(publicAPI, model) {
  ...
  publicAPI.compileShader = () => {
    if (!model.vertexShader.compile()) {
      vtkErrorMacro3(model.vertexShader.getSource().split("\n").map((line, index) => `${index}: ${line}`).join("\n"));
      vtkErrorMacro3(model.vertexShader.getError());
      return 0;
    }
    if (!model.fragmentShader.compile()) {
      vtkErrorMacro3(model.fragmentShader.getSource().split("\n").map((line, index) => `${index}: ${line}`).join("\n"));
      vtkErrorMacro3(model.fragmentShader.getError());
      return 0;
    }
    if (!publicAPI.attachShader(model.vertexShader)) {
      vtkErrorMacro3(model.error);
      return 0;
    }
    ...
    if (!publicAPI.link()) {
      vtkErrorMacro3(`Links failed: ${model.error}`);
      return 0;
    }
  ...
  publicAPI.link = () => {
    ...
    if (!isCompiled) {
      ...
      vtkErrorMacro3(`Error linking shader ${lastError}`);
      model.handle = 0;
      return false;
    }
  ...
  publicAPI.isUniformUsed = (name) => {
    ...
    if (!model.linked) {
      vtkErrorMacro3("attempt to find uniform when the shader program is not linked");
      return false;
    }
    loc = model.context.getUniformLocation(model.handle, name);
    model.uniformLocs[name] = loc;
...
var newInstance6 = macro.newInstance(extend5, "vtkShaderProgram");
...
function vtkOpenGLCellArrayBufferObject(publicAPI, model) {
  ...
  publicAPI.setCoordShiftAndScale = (coordShift, coordScale) => {
    if (coordShift !== null && (coordShift.constructor !== Float64Array || coordShift.length !== 3)) {
      vtkErrorMacro4("Wrong type for coordShift, expected vec3 or null");
      return;
    }
    if (coordScale !== null && (coordScale.constructor !== Float64Array || coordScale.length !== 3)) {
      vtkErrorMacro4("Wrong type for coordScale, expected vec3 or null");
      return;
    }
...
var newInstance9 = macro.newInstance(extend8, "vtkOpenGLVertexArrayObject");
...
var newInstance11 = newInstance(extend10, "vtkOpenGLTexture");
var vtkOpenGLTexture$1 = {
  newInstance: newInstance11,
  extend: extend10,
  ...Constants2
};
registerOverride("vtkTexture", newInstance11);
...
//# sourceMappingURL=chunk-QMFPSI3R.js.map
